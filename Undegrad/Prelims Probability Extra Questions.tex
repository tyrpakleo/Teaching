...
\subsection{Sheet 5: Markov Chains}
Notes:
\begin{itemize}
    \item Questions 1,2,3,5,6 can all be put into the framework of Markov Chains. Markov chains are probabilistic objects of a system evolving over time where the evolution only depend on the current state, not on the whole past. We call states terminal/ absorbing if the chain stays at that state when it is there. An equilibrium distribution is one where if the chain has this distribution then next step it has that same distribution. This is discussed more thoroughly in Part A(2n year) Probability.
    \item Q1 is about finding the equilibrium of that Markov chain and finding out that we have exponential convergence to equilibrium.
    \item Q2 is about the expected time to reach the absorbing state of the Markov chain.
    \item Q3 is about the expected time to reach the absorbing states of the Markov chain.
    \item Q6 (a) is about finding the expected covering time of the Markov chain, i.e. the expected amount of steps required to visit every state of the Markov chain.
\end{itemize}

Extra:
\begin{itemize}
    \item Q1,2,3,5,6: draw the associated Markov chain. What are the absorbing states for each Markov chain? What are the equilibrium distributions for each Markov chain? What is their transition matrix?
    \item Q1: consider the same question with a square, pentagon then \(n\)-gon, what do you get?
    \item Q3: consider the same questions with probability \(p\) of winning \(£1\) and probability \(1-p\) of winning \(£1\). What about probability \(1/3\) of getting \(\{-1,1,2\}\) after each round?
    \item Q5: consider the same questions with a probability \(p\) of Heads. Can we characterise the distribution of \(Y\) easily.
    \item Q6: Suppose that we are given this question on an undirected graph, where at each step we travel along one of the edges of the graph to a neighbour. In this particular case the graph was a cycle, but we can consider other graphs. Prove that if the distribution of the last vertex to be visited is uniform then we are on a cycle or a complete graph.
\end{itemize}
...
We will consider a biased coin with probability \(p\) of Heads.
\begin{itemize}
...
\end{itemize}
\subsection{Sheet 6: Generating functions}
Notes:
\begin{itemize}
    \item Q6 (d): 
\end{itemize}
...
Extra:
\begin{itemize}
    \item Q2: prove that the sum of \(m\) independent Geometric random variables with parameter \(p\) has the distribution of a negative Binomial (e.g. prove both have same probability mass function).
    \item Q3: prove that the formulae in part (a) hold assuming only that \(\{N\geq k\}\) is independent of \(X_k\).
    \item Q4: find an expression involving generating functions for the probability that \(n\) divides \(X\in\Z_+\).
    \item Q5: find the probability that there is \(1,2,3\) individuals after \(2\) minutes.
    \item Q6: prove that the probability the process eventually dies out is the smallest non-negative solution to \(G(s)=s\) (Hint: consider \(\alpha_n=\p{X_n=0}\) and let \(n\to\infty\)).
\end{itemize}
\subsection{Sheet 7: Continuous variables}

Extra:
\begin{itemize}
    \item Does there exist a function with a root at \(0,1/2\) which is always positive. How can we tell if there is a sign change at a root. Give a complete characterisation and prove it is correct.
    \item Q2: is there normalising constant for \(x^\alpha(1-x)^{\beta}\) for \(x\in(0,1)\)? If so find it.
    \item Q3: Find \(\mathbb{E}[U^k],\mathbb{E}[\exp(\lambda U)]\)
\end{itemize}
Grimmett:
\begin{itemize}
    \item All 4.1, 4.3
\end{itemize}
\subsection{Sheet 8: Continuous variables}

Extra:
\begin{itemize}
    \item Q2: give a few more terms of the asymptotic expansion of the quantity you get and give precise error bounds (Hint:Taylor). Now calculate \(\mathbb{P}(M_n\leq\log(n)+c)\) as a function of \(c\). Does this tend to anything as \(n\to\infty\). If it does, find the \(c\) such that \(\lim_{n\to\infty}\mathbb{P}(M_n\leq\log(n)+c)=\frac{1}{2}\).
    \item Q4: use other means to get a more precise upper bound or an exact quantity. (Hint: MGF) What sort of bound would the central limit theorem tell us we should expect.
    \item Q6: Explain intuitively why we have a lower variance for \(d\) than for \(c\). Does lower variance, same mean imply concentration around the mean? Why? Calculate higher order moments for the random variables in part \(c,d\), at least to leading order of \(n\). Then see if the scaling is correct so that we should expect a central limit theorem to hold. 
    \item Q7: What happens when \(p=a,b\)? Prove the law of large numbers for Bernoulli random variables. How would you start this question? Does your answer change if we consider \([a,b),(a,b]\). What is \(\mathbb{P}(S_n=\lfloor np\rfloor)\) as \(n\to\infty\).
\end{itemize}
...
Prove that \(\lim_{n\to\infty}\alpha_{n,k}=\E{N^k}\). 
This goes towards proving the Central Limit theorem by proving the moments converge, this is called the method of moments. 
This also appears in the \(4\)'th year course random matrix theory and in Terence Tao's book(https://terrytao.wordpress.com/wp-content/uploads/2011/02/matrix-book.pdf).
